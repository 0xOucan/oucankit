...
export class AaveProtocolActionProvider extends ActionProvider {
  ...
  async borrowAsset(
    ...
    try {
      ...
      const amount = BigInt(args.amount);
      const interestRateMode = INTEREST_RATE_MODES[args.interestRateMode];
   ...
   */
  private async getUserReserveData(userAddress: string, assetAddress: string): Promise<UserReserveData> {
    return this.publicClient.readContract({
  ...
  private async canUserWithdraw(
    ...
    if (accountData.totalDebtBase > 0n) {
      ...
      const newHealthFactor = (newCollateral * accountData.currentLiquidationThreshold) / (accountData.totalDebtBase * 10000n);
